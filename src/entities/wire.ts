...
export class WireTemplate extends BaseTemplate<WireTemplateType, PortTemplateType> {
  makeSprite({portAId, portBId, collection}: WireTemplateParams): PixiSprite {
    ...
    const pAPos = (pAEnt.toParent()?.entity as System).components.position as Rect;
    const pBPos = (pBEnt.toParent()?.entity as System).components.position as Rect;
